...
static func _retrieve_node_meta_from_self(property_name : String, from_node : Node) -> Variant:
	if(from_node == null):
		push_error("Try to retrieve metadata from invalid node")
		return null
...
static func _add_node_meta_to_self(property_name : String, from_node : Node, data_to_set : Variant) -> void:
	if(from_node == null):
		push_error("Cannot add metadata to null parent!")
		return
